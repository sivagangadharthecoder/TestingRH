...
const AdminIdRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/id-requests?page=${page}&limit=${limit}&search=${search}`
      );
  ...
  const handleStatusChange = async (id, status) => {
    try {
      ...
      await axios.patch(
        `http://localhost:5000/api/id-requests/${id}`,
        { status }
      );
  ...
  return (
    <div className="zara-admin-container">
      ...
      <div className="zara-admin-header">
        <h1 className="zara-admin-title">ID Card Requests</h1>
