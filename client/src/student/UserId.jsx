...
const UserId = () => {
    ...
    const fetchRequests = async () => {
        try {
            ...
            const response = await axios.get(
                `http://localhost:5000/api/id-requests?page=${page}&limit=${limit}`
            );

            // Filter requests by user's roll number
            ...
            setPagination(prev => ({
                ...
                // Note: You might want to adjust these pagination values
                // since they reflect the unfiltered total
            }));
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this request?')) {
            try {
                await axios.delete(`http://localhost:5000/api/id-requests/${id}`);
    ...
    const downloadPdf = (id) => {
        window.open(`http://localhost:5000/api/id-requests/pdf/${id}`, '_blank');
    ...
    return (
        <div className="zara-admin-container">
            ...
            <div className="zara-admin-header">
                <h1 className="zara-admin-title">Your ID Card Requests</h1>
